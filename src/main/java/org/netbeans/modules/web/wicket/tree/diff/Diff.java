...
/**
 * Represents the difference between two lists. The <code>getChanges()</code>
 * method returns a list of <code>Change</code> objects, which, if applied to
 * the old list in the order they are returned, will result in the new list.
 * Example code for such a transformation:
 * <pre>
 ...
 *     }
 * }
 * </pre> Note that the start/end offsets of Change objects in a diff take into
 * account earlier changes. In other words, if element 1 of a list was changed
 * and element 0 of a list was removed, there will be a deletion at index 0 and
...
public abstract class Diff<T> {
    ...
    private static final boolean VALIDATE = Boolean.getBoolean(
            "org.netbeans.misc.diff.validate");

    /**
     * Get a list of Change objects in the order they need to be applied to
     * construct the result of <code>getNew()</code> by applying these changes
     * to <code>getOld()</code>.
     *
    ...
    public static <T> Diff<T> create(List<T> old, List<T> nue) {
        Diff<T> result = new ListDiff<>(old, nue);
        if (VALIDATE) {
            assert validDiff(result);
        }
        return result;
    }
     ...
     * @return The created Diff object
     */
    ...
    private static boolean validDiff(Diff diff) {
        List<String> list = new ArrayList<>(diff.getOld());
